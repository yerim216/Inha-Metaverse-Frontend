...
import { addJob, addMember, createTeam, getJobs, getProjectCategory } from '../APIs/team';
...
export default function CreateProject() {
   ...
   const handleJobInputTitle = () => {
      if (jobSelectRef.current && jobSelectRef.current.options.selectedIndex >= 0) {
         let newJobInput = { ...jobInput };
         newJobInput.title = jobSelectRef.current.options[jobSelectRef.current.selectedIndex].getAttribute('jobtitle');
   ...
   const getUserName = async () => {
      ...
      const userIndex = JSON.parse(localStorage.getItem('recoil-persist')).userState.user_index;
   ...
   useEffect(() => {
      ...
      userIndex = JSON.parse(localStorage.getItem('recoil-persist')).userState.user_index;
   ...
   return (
      <>
         {judgeModalOpen && (
            <div className={`${judgeModalOpen ? 'openModal modal' : 'modal'} flex justify-center items-center`}>
               <div
                  ref={modalRef}
         ...
         >
                              ...
                              return (
                                 <CategoryBtn
                                    ...
                                    selected={selectedCategory.some(
                                       (obj) => obj.category_index === category.category_index
                                    )}
                                    handleAddCategory={() => {
                                    ...
                                    handleDeleteCategory={(index) => {
                                       setSelectedCategory((cur) => {
                                          return cur.filter((category) => category.category_index !== index);
                                       });
                                    }}
                                 />
                              );
                  ...
                  >
                     ...
                     <div className="w-[55%] h-5/6 flex flex-col justify-between">
                        <div className={styles.recruitmentBox_leftArea}>
                           ...
                           <div className={`${styles.recruitmentBox_inputBox} flex gap-6`}>
                              <div
                              ...
                              >
                                 <select
                                    ...
                                    className="w-[100%] h-full bg-transparent outline-none appearance-none"
                                    onChange={(e) => {
                                       handleJobInputRecruitmentNum(parseInt(e.target.value));
                                    }}
                                 >
            ...
            <div className="flex w-full justify-end gap-8 mt-36">
               <button
                  ...
                  onClick={async (e) => {
                     if (
                        ...
                        alert('입력되지 않은 정보가 존재합니다(프로젝트 분야, 모집 제외). 다시 확인해주세요!');
                        return;
                     }
               ...
               <button
                  ...
                  onClick={(e) => {
                     e.preventDefault();
                     const returnVal = window.confirm('작업을 취소하고 되돌아가시겠습니까?');
                     if (returnVal === true) {
                        window.history.back();
                     }
                  }}
