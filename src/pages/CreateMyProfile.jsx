...
export default function CreateMyProfile() {
   ...
   const getField = () => {
      getUserInterested()
         .then(function (response) {
            ...
            const uniqueCategories = [...new Set(categories.map((category) => category.field_category))];
            ...
            if (field.length < 4) setField((field) => [...field, { 기획: plannig }]);
            ...
            if (field.length < 4) setField((field) => [...field, { 디자인: designs }]);
            ...
            if (field.length < 4) setField((field) => [...field, { 개발: develops }]);
   ...
   const getCurrentDate = () => {
      const now = new Date();
      const currentYear = parseInt(now.getFullYear());
      const currentMonth = parseInt(now.getMonth() + 1); // 월은 0부터 시작하므로 1을 더해줍니다.
   ...
   const handleCareerChange = () => {
      ...
      const dateDifference = calculateDateDifference(syear, smonth, eyear, emonth);
      ...
      const isCurrentJob = eyear === currentYear && emonth === currentMonth;
      console.log(eyear);
   ...
   const getContains = (skillName) => {
      ...
      for (step = 0; step < skillSearch.length; step++) {
         if (skillName[step].toUpperCase() !== skillSearch[step].toUpperCase()) return false;
      }
      return true;
   };
   ...
   const handleClickSave = () => {
      ...
      if (detailCareer) {
         detailCareer.map((career) => {
            console.log(career);
            putUserCareer(userIdx, career.job, career.syear, career.smonth, career.eyear, career.emonth)
               .then(function (res) {
      ...
      if (selectedSkills) {
         selectedSkills.map((skill) => {
            putUserSkill(userIdx, skill.skill_index)
               .then(function (res) {
         ...
         <div className={styles.wrap}>
            ...
            <div className={styles.topWrap}>
               ...
               <div className={styles.nameWrap}>
                  ...
                  >
                     <select className={styles.customInputBox} value={selectedField} onChange={handleSelectField}>
                        {field.map((item, index) => {
                           // 각 인덱스의 key 값을 가져옴
                           const keys = Object.keys(item);

                           return keys.map((key) => (
                              <option key={index} value={key} style={{ color: tm.textColor }}>
                                 {key}
                              </option>
                           ));
                        })}
                  ...
                  >
                     <select className={styles.customInputBox} value={selectedJob} onChange={handleSelectJob}>
                        {field.map((item, index) => {
                              ...
                              return (
                                 <>
                                    ...
                                    {selectedArray.map((selectedItem, selectedIndex) => (
                                       <option key={selectedIndex} value={selectedItem.field_index}>
               ...
               <div className={styles.career}>
                  <div className={styles.careerIn}>
                     ...
                     <input
                        ...
                        value={job}
                        onChange={(e) => handleSetCareer(e.target.value)}
                        placeholder="직장명을 입력해주세요."
                     ></input>
                     <ErrorMsg style={{ bottom: '-20px' }} errMsg={errorMessages.jobName} />
                  </div>
                  <div className={styles.careerIn}>
               ...
               <div className={styles.resultWrap}>
                  {detailCareer &&
                     detailCareer.map((career, index) => {
                        ...
                        const isCurrentJob = career.eyear === curYear && career.emonth === curMonth;

                        return (
                           <div className={styles.resultInner}>
                              <div className={styles.result} style={{ background: tm.inputBg, color: tm.textColor }}>
                                 <div className={styles.jobName} style={{ color: tm.textColor }}>
                                    {career.job}
                                 </div>
                                 <div className={styles.dateTxt}>
                                    ...
                                    {isCurrentJob ? (
                                       ...
                                       <>
                                          <div className={styles.dateDiffer} style={{ background: tm.careerBg }}>
                                             {career.dateDifference.years !== 0 ? (
                                                <div className={styles.dateDifTxt} style={{ color: tm.textColor }}>
                                                   {career.dateDifference.years}년
                                                </div>
                                             ) : null}

                                             <div className={styles.dateDifTxt} style={{ color: tm.textColor }}>
                                                {' '}
                                                {career.dateDifference.months}개월
                                             </div>
                                          </div>
                                       </>
                                    )}
