...
export default function CalDayGrid(props) {
  ...
  function getNextSundayFromDate(date, weeks) {
    ...
    // 6주를 일요일 기준으로 더함
    nextSunday.setDate(
      nextSunday.getDate() + (6 - currentDayOfWeek) + 7 * weeks + 1
    );
                  ...
                  if (index === 0) {
                    ...
                    filteredEvents.forEach((schedule, index) => {
                      ...
                      for (
                        ...
                        for (
                          let i = 1;
                          i < scheduleByDate[dateString].length;
                          i++
                        ) {
                          if (
                            todayEvents[i].schedule_priority ===
                            todayEvents[i - 1].schedule_priority
                          ) {
                  ...
                  if (index === 0) {
                    {
                      scheduleByDate[dateToString] &&
                        scheduleByDate[dateToString].map((event, indexes) => {
                          ...
                          for (
                            let i = 1;
                            i < scheduleByDate[dateToString].length;
                            i++
                          ) {
                            if (
                              todayEvents[i].schedule_priority ===
                          ...
                          const formatStartDate = event.start_date.substring(
                            0,
                            10
                          );
                          const formatEndDate = event.end_date.substring(0, 10);
                          ...
                          if (maxBarLen < eventLeng[0]) {
                            ...
                            for (let j = 0; j < eventBarCol; j++) {
                              const resultDate = getNextSundayFromDate(
                                dateForStart,
                                j
                              ); // j주 이후의 일요일 날짜
                              const formatResult =
                                formatDateToYYYYMMDD(resultDate); //yyyy-mm-dd
                              ...
                              weekCut[formatResult].sort(
                                (a, b) =>
                                  a.schedule_priority - b.schedule_priority
                              );
                            }
                          }
                          ...
                          const formattedDStart = moment(
                            formatStartDate,
                            "YYYY-MM-DD"
                          ).toDate();
                          const nextDay = moment(formattedDStart)
                            .add(1, "days")
                            .toDate();

                          const formattedDEnd = moment(
                            formatEndDate,
                            "YYYY-MM-DD"
                          ).toDate();
                          const nextEDay = moment(formattedDEnd)
                            .add(1, "days")
                            .toDate();
                      ...
                      if (dateKey === dateToString) {
                        ...
                        const todayToDate = new Date(dateToString);
                        {
                          event.map((events, index) => {
                            let eventBarLen = 0;
                            const formatStartDates =
                              events.start_date.substring(0, 10);
                            const formatEndDates = events.end_date.substring(
                              0,
                              10
                            );
                            ...
                            const daysDiff =
                              Math.floor(timeDiff / (1000 * 60 * 60 * 24)) + 1; //이벤트 길이 반환
                            ...
                            const nextEDay = moment(formattedDStart)
                              .add(1, "days")
                              .toDate();
              ...
              {dayEventLast.map((events, index) => {
                ...
                if (clickedRow !== -1) {
                  ...
                  if (index === 0) {
                    ...
                    events.priority != index + 1 &&
                    events.priority - priorityCheck != 1
                  ) {
                  ...
                  return (
                    <>
                      {emptyBoxes}
                      {Array.from(
                        { length: Math.min(remainingItems, 1) },
                        (_, i) => (
                          <>
                            <span
                              key={index}
