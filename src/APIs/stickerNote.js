...
export function createStickerMemo({ teamIndex, content, writerIndex, x, y, size_x, size_y, color }) {
...
export function modifySticker(stickerInfos) {
   ...
   const data = stickerInfos.map((item) => {
      const { team_name, note_writer, user_name, team_index, created_at, ...newItem } = item;
      return newItem;
   });
